...
| **FR-15** | Manajemen Bentuk Sangkar Reaktif | Admin dapat menambah bentuk sangkar baru melalui dialog pop-up yang langsung disinkronkan secara global (`CageService`). Dilengkapi navigasi panah `<` & `>`, scrolling roda mouse (mouse wheel horisontal), mouse drag, scrollbar kustom, dan auto-scroll otomatis ke item baru. |

...
| **FR-27** | Navigasi Footer Bawah Layar & Header Ringkas Mobile | Pada tampilan mobile (< 768px), antarmuka menyajikan footer navigasi bawah layar (`bottomNavigationBar`) dengan menu Keranjang (badge count), Beranda (tombol bulat emas di tengah), dan Profil. Tombol keranjang dan profil di TopNavbar disembunyikan agar search bar dapat melebar secara leluasa. |
| **FR-28** | Navigasi Footer & Header Bersih Admin Mobile | Pada tampilan mobile, dashboard admin menyajikan header minimalis tanpa tombol menu hamburger `☰` dan tanpa ikon profil di header, serta memindahkan navigasi ke footer bar bawah (Setting di kiri, Preview Umum / Marketplace di tengah, Profil di kanan). |
| **FR-29** | Interactive Fullscreen Product Viewer | Mengklik atau mengetuk gambar foto produk maupun foto bentuk sangkar pada detail produk membuka viewer layar penuh (`ProductFullscreenViewer`) dengan dukungan navigasi slide antar gambar (`<` dan `>`), pinch-to-zoom, double-tap zoom, mouse drag & wheel, serta pan interaktif di mobile dan desktop. |
| **FR-30** | Pembersihan & Perlindungan Duplikasi Master Bentuk Sangkar | Sistem `CageService` dan Supabase `bentuk_sangkar` menjamin data master sangkar bersih dari data dummy (8 item dummy lama dibersihkan), menerapkan deduplikasi ID dan nama saat fetch, serta mengeksekusi penghapusan cloud secara asynchronous terjamin (`await`) agar data yang dihapus tidak muncul kembali saat reload. |
| **FR-31** | Standardisasi Banner & Tata Letak Tombol Navigasi Member | Halaman beranda member private (`UserHomePage`) mengadopsi `HeroBanner` penuh dengan tombol *"Katalog Umum"* bergaya dark-glass yang menimpa banner di sudut kanan atas. Tombol aksi utama *"Permintaan Logo Custom"* ditempatkan tepat di atas katalog logo custom sebelah kanan. Saat admin atau member melihat katalog standar, banner tetap ditampilkan penuh dan tombol navigasi kembali diletakkan di bawah banner dan di atas katalog sebelah kanan. |
| **FR-32** | Hak Akses Admin Produk Custom User (Read & Edit Only) | Admin dibatasi hanya untuk meninjau dan menyunting gambar atau spesifikasi logo custom yang telah diajukan oleh pengguna private via `AdminEditProductPage`. Seluruh tombol dan formulir penambahan manual produk custom di sisi admin ditiadakan agar inisiasi desain selalu bersumber dari pengajuan mandiri pelanggan. |
...
| **FR-36** | Asisten AI Percakapan Teks & Visual Cerdas | Pengguna dapat berkonsultasi mengenai produk sangkar burung melalui dialog asisten AI interaktif (`AiAssistantDialog`) dengan respons teks cepat dan kartu visual. Sistem menyaring sapaan umum/chit-chat kasual tanpa menampilkan kartu produk tidak relevan, mendeteksi pertanyaan konsultasi/opini murni secara objektif tanpa kartu produk, serta mendukung rekomendasi lanjutan ("kalau yang Excellent?") dan alternatif ("cari yang lain") yang mengecualikan produk yang telah disarankan sebelumnya. Fitur audio TTS telah ditiadakan agar percakapan tetap hening dan nyaman. |

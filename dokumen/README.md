...
└── backend/                       # Dokumentasi server-side & database (Supabase)
    ├── README.md                  # Konfigurasi Supabase, autentikasi role, & API
    └── skema_database.md          # Struktur tabel (produk, bentuk_sangkar, pesanan, user_private), relasi, & RLS
```
...
| :--- | :--- | :--- |
| **Planing** | Rencana proyek, ruang lingkup, target pengguna, kebutuhan fungsional (FR-01 s/d FR-21) & non-fungsional, alur pengguna, serta roadmap rilis. | [Lihat Dokumen Planing](./planing/README.md) |
...
| **Backend** | Integrasi Backend-as-a-Service (BaaS) Supabase, konfigurasi URL & API Key, skema database PostgreSQL (tabel produk, bentuk sangkar, pesanan 4 tahap, pengaturan toko), dan aturan keamanan (RLS). | [Lihat Dokumen Backend](./backend/README.md) |
